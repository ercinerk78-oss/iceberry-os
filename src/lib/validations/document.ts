...
export function validateDocumentFiles(files: File[], documentType: DocumentType): string | null {
  ...
  if (files.some((file) => !ALLOWED_MIME_TYPES.includes(file.type as (typeof ALLOWED_MIME_TYPES)[number]))) {
  ...
  if (documentType === "LOCATION_ANALYSIS_PDF" && files.some((file) => file.type !== "application/pdf")) {
    return "Ana rapor alanına yalnızca PDF yükleyebilirsiniz.";
  }
  if (documentType === "LOCATION_ANALYSIS_VISUAL" && files.some((file) => !["image/jpeg", "image/png"].includes(file.type))) {

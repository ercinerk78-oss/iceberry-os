...
const schema = z.object({
  version: z.string().trim().min(1, "Versiyon zorunludur.").max(30, "Versiyon en fazla 30 karakter olabilir."),
...
export async function uploadCandidateDocuments(candidateId: string, requestedType: DocumentType, _state: DocumentActionState, formData: FormData): Promise<DocumentActionState> {
  ...
  try {
    for (const file of files) stored.push({ file, ...(await storage.save(file)) });
    await prisma.$transaction(stored.map(({ file, fileName, filePath }) => prisma.document.create({
      data: {
        fileName,
...
export async function uploadRelatedDocuments(relation: "franchisee" | "branch" | "opening", relationId: string, requestedType: DocumentType, _state: DocumentActionState, formData: FormData): Promise<DocumentActionState> {
  ...
  const legacyOpening = relation === "opening" ? await prisma.branchOpening.findFirst({ where: { id: relationId, archivedAt: null }, select: { id: true } }) : null;
  const projectOpening = relation === "opening" && !legacyOpening ? await prisma.openingProject.findFirst({ where: { id: relationId, archivedAt: null }, select: { id: true } }) : null;
  const exists = relation === "franchisee"
    ? await prisma.franchisee.findFirst({ where: { id: relationId, archivedAt: null } })
    : relation === "branch"
      ? await prisma.branch.findFirst({ where: { id: relationId, archivedAt: null } })
      : legacyOpening || projectOpening;
  ...
  try {
    ...
    await prisma.$transaction(stored.map(({ file, fileName, filePath }) => prisma.document.create({
      data: {
        ...
        version: parsed.data.version,
        description: parsed.data.description || null,
        franchiseeId: relation === "franchisee" ? relationId : null,
        branchId: relation === "branch" ? relationId : null,
        openingId: legacyOpening ? relationId : null,
        openingProjectId: projectOpening ? relationId : null,
        uploadedBy: "Sistem Kullanıcısı",
      },
    })));
  } catch {
  ...
  revalidatePath("/documents");
  return { success: true, message: "Doküman başarıyla yüklendi." };
}

export async function archiveDocument(documentId: string, formData: FormData) {
  void formData;

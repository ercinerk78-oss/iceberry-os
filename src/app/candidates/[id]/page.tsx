...
export default async function CandidateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.franchiseCandidate.findFirst({ where: { id, archivedAt: null }, include: { interactions: { orderBy: { interactionDate: "desc" } }, tasks: { orderBy: { dueDate: "asc" } }, documents: { orderBy: { createdAt: "desc" } } } });
  if (!record) notFound();

...
export default async function CandidateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.franchiseCandidate.findFirst({ where: { id, archivedAt: null }, include: { interactions: { orderBy: { interactionDate: "desc" } }, tasks: { orderBy: { dueDate: "asc" } }, documents: { orderBy: { createdAt: "desc" } }, franchisee: { select: { id: true, companyName: true } } } });
  if (!record) notFound();
  ...
  return <AppShell activeHref="/candidates" eyebrow="Franchise aday detayı" title={candidate.fullName}><div className="space-y-4">
    ...
    <CandidateConversion candidate={candidate} franchisee={record.franchisee}/>
